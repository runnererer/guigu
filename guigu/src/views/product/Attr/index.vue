          ...
          <el-table-column  width="width" prop="prop"  label="属性值名称">
            <template slot-scope="{row,$index}">
              ...
              <el-input v-model="row.valueName" placeholder="请输入属性值名称" size="mini" v-if="row.flag" @blur="toLook(row)"
               @keyup.native.enter="toLook(row)" :ref="$index"> 
               </el-input>
  ...
  methods: {
    ...
    addAttrValue(){
      ...
      this.attrInfo.attrValueList.push({
        attId:this.attrInfo.id, //对于修改某一个属性的时候,可以在已有的属性值基础上新增新的属性值(新增属性值的时候,需要把已有的属性的id带上)
        valueName:'',
        flag:true
      });
   ...
   updateAttr(row){
    ...
    toEdit(row,index){
      row.flag = true;
      //获取input节点,实现自动聚焦
      //因为index是变量,所以不能是this.$refs.index 而要使用枚举的方式 this.$refs[index]
      //点击span的时候,重绘重拍一个input他是需要耗费时间,因此我们不可能一点击span立马获取input
   ...
   async addOrUpdateAttr(){
      //整理参数:1,如果用户添加过很多属性值,且属性值为空的不应该提交给服务器
